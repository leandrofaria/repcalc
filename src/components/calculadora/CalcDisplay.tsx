...
const CalcDisplay = ({ state }: { state: CalcState }) => (
  <div>
    <output
      className="history block w-full mb-3 h-[36px] leading-[36px] px-3 bg-slate-400 border-[1px] border-[#c6d1df] shadow-inner text-lg font-semibold text-right text-white overflow-hidden"
      aria-label="Memória e operação pendente"
    >
      {memoryLine(state)}
    </output>
    <output
      className="display flex w-full h-[69px] border-[1px] border-[#c6d1df] bg-white shadow-inner p-3 flex-col justify-center items-end overflow-hidden text-3xl text-right"
      aria-live="polite"
    ...
    {state.error !== null && (
      <p role="alert" className="mt-3 text-red-600 font-semibold text-center">
